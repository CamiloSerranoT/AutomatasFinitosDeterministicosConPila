...
public class Funciones extends JFrame implements ActionListener {
    ...
    public int getBotonesActivos() {
        return botonesActivos;
    }

    public void setBotonesActivos(int botonesActivos) {
        this.botonesActivos = botonesActivos;
    }
    ...
    public void crearSubPanel() {
        ...
        botones = new JButton[vista.getEstados() * vista.getEstados()];

        for (int i = 0; i < vista.getEstados() * vista.getEstados(); i++) {
            texto = new JButton();
                    ...
                    if (i == 0 && j > 0) {
                        ...
                        if (j == 0 && i > 0) {
                            ...
                            if (matrizAux[i - 1][j - 1].charAt(0) - 48 == 1) {
                                botones[cont].setText("" + 1);
                                botones[cont].setEnabled(true);
                                jPanel2.add(botones[cont]);
                                cont++;
                                setBotonesActivos(getBotonesActivos() + 1);
                            } else {
                                botones[cont].setText("" + 0);
                                jPanel2.add(botones[cont]);
                                cont++;
                            }
                        }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        boolean com = false;
        if (e.getSource() == jb1) {
            control.volverAMenu(this);
        } else {
            for (int i = 0; i < vista.getEstados() * vista.getEstados(); i++) {
                if (e.getSource() == botones[i]) {
                    Lista lista = new Lista(this, i, control, botones[i]);
